...
\begin{frame}{Matriz de Transición: propiedades}
    \small
...
\begin{frame}{Ejemplo Introductorio: ¿Qué pasa en 2 días?}
    ...
    Esto equivale a calcular $P^2 = P \cdot P$: \textbf{no} es elevar cada número al cuadrado, sino \emph{multiplicar la matriz por sí misma}. Veamos la cuenta en la próxima slide.
\end{frame}

\begin{frame}{Ejemplo Introductorio: La cuenta de $P^2$ paso a paso}
    \small
    Para multiplicar dos matrices, cada casillero del resultado es \textbf{fila $\times$ columna}: se multiplica entrada a entrada y se suma. El orden de filas/columnas es (F, R).

    \vspace{0.5em}

    \textbf{Casillero por casillero:}
    \begin{itemize}
        \item $(F,F)$: fila F $\times$ col. F $= 0.8 \cdot 0.8 + 0.2 \cdot 0.6 = 0.64 + 0.12 = \mathbf{0.76}$
        \item $(F,R)$: fila F $\times$ col. R $= 0.8 \cdot 0.2 + 0.2 \cdot 0.4 = 0.16 + 0.08 = 0.24$
        \item $(R,F)$: fila R $\times$ col. F $= 0.6 \cdot 0.8 + 0.4 \cdot 0.6 = 0.48 + 0.24 = 0.72$
        \item $(R,R)$: fila R $\times$ col. R $= 0.6 \cdot 0.2 + 0.4 \cdot 0.4 = 0.12 + 0.16 = 0.28$
    \end{itemize}
    ...
    El casillero $(F,F)$ es \emph{exactamente} la suma de los caminos $F\to F\to F$ y $F\to R\to F$.
\end{frame}
...
\begin{frame}{Ejercicio 2: Conclusión}
    \small
    \textbf{Conclusión:} Cerúndolo tiene un 76.8\% de probabilidad de ganar el punto y consagrarse campeón.

    \vspace{1em}

    \textit{A largo plazo:} Si jugaran infinitos puntos con estas probabilidades, Cerúndolo ganaría el 76.8\% y Rune el 23.2\%.
\end{frame}
...
\begin{frame}{Chapman-Kolmogorov: ejemplo numérico}
    \small
    Con la cadena del clima (Ej. 1), $P = \begin{pmatrix} 0.7 & 0.2 & 0.1 \\ 0.3 & 0.4 & 0.3 \\ 0.2 & 0.4 & 0.4 \end{pmatrix}$, orden (S, N, L).

    \vspace{0.5em}

    Queremos $P_{SL}^{(2)}$: ir de Soleado a Lluvia en 2 pasos. Sumamos sobre el estado intermedio $k$ del día 1:
    \[
    P_{SL}^{(2)} = \underbrace{P_{SS}\,P_{SL}}_{S\to S\to L} + \underbrace{P_{SN}\,P_{NL}}_{S\to N\to L} + \underbrace{P_{SL}\,P_{LL}}_{S\to L\to L}
    \]
    \[
    ...
    Cada término es un camino de 2 pasos. Esto es justo el casillero $(S,L)$ de $P^2$: calcular $P^2$ es hacer esta suma para todos los pares de estados.
\end{frame}
...
\begin{frame}{Ejercicio 4: Solución - Planteo}
    \small
    \textbf{Paso 1:} Matriz de transición (orden B, M, A)
    ...
    \textbf{Paso 2:} Estado inicial $\mathbf{e_0} = [1\ 0\ 0]$ (100\% en baja).

    \vspace{0.5em}

    Como $\mathbf{e_0} = [1,0,0]$, multiplicar por $\mathbf{e_0}$ \textbf{selecciona la primera fila}. Buscamos entonces la \textbf{primera fila de $P^3$}. Vamos por etapas: $P^2 = P\cdot P$, luego $P^3 = P^2 \cdot P$.
\end{frame}

\begin{frame}{Ejercicio 4: Solución - Paso a paso}
    \small
    \textbf{Fila B de $P^2$} (fila B de $P$ por cada columna):
    \begin{itemize}
        \item $(B,B)$: $0.3 \cdot 0.3 + 0.5 \cdot 0.2 + 0.2 \cdot 0.3 = 0.25$
        \item $(B,M)$: $0.3 \cdot 0.5 + 0.5 \cdot 0.4 + 0.2 \cdot 0.5 = 0.45$
        \item $(B,A)$: $0.3 \cdot 0.2 + 0.5 \cdot 0.4 + 0.2 \cdot 0.2 = 0.30$
    \end{itemize}
    $\Rightarrow$ Fila B de $P^2 = [0.25,\ 0.45,\ 0.30]$ \emph{(el $0.30$ es a 2 días, no es lo pedido).}
\end{frame}

\begin{frame}{Ejercicio 4: Solución - Paso a paso (cont.)}
    \small
    Ya tenemos la fila B de $P^2 = [0.25,\ 0.45,\ 0.30]$. Ahora $P^3 = P^2 \cdot P$.

    \vspace{0.4em}

    \textbf{Fila B de $P^3$} ($[0.25, 0.45, 0.30]$ por cada columna de $P$):
    \begin{itemize}
        \item $(B,B)$: $0.25 \cdot 0.3 + 0.45 \cdot 0.2 + 0.30 \cdot 0.3 = 0.255$
        \item $(B,M)$: $0.25 \cdot 0.5 + 0.45 \cdot 0.4 + 0.30 \cdot 0.5 = 0.455$
        \item $(B,A)$: $0.25 \cdot 0.2 + 0.45 \cdot 0.4 + 0.30 \cdot 0.2 = 0.290$
    \end{itemize}

    \vspace{0.4em}

    $\mathbf{e_0}\cdot P^3 = [0.255,\ 0.455,\ \boxed{0.290}]$ \quad (suman 1 \checkmark)

    \textbf{Respuesta:} hay 29\% de probabilidad de estar en alta demanda en 3 días.
\end{frame}
...
\begin{frame}{Ejercicio 5: Solución Parte a)}
    \small
    Matriz $P = \begin{pmatrix} 0.1 & 0.3 & 0.6 \\ 0.2 & 0.2 & 0.6 \\ 0.2 & 0.4 & 0.4 \end{pmatrix}$ (orden A, B, C).

    \vspace{0.4em}

    \textbf{Estado inicial:} $\mathbf{e_2} = [0\ 0\ 1]$ (hoy está en C). Multiplicar por $\mathbf{e_2}$ \textbf{selecciona la tercera fila}: buscamos la fila C de $P^2$.

    \vspace{0.4em}

    \textbf{Fila C de $P^2$} (fila C $= [0.2, 0.4, 0.4]$ por cada columna):
    \begin{itemize}
        \item $(C,A)$: $0.2 \cdot 0.1 + 0.4 \cdot 0.2 + 0.4 \cdot 0.2 = 0.18$
        \item $(C,B)$: $0.2 \cdot 0.3 + 0.4 \cdot 0.2 + 0.4 \cdot 0.4 = 0.30$
        \item $(C,C)$: $0.2 \cdot 0.6 + 0.4 \cdot 0.6 + 0.4 \cdot 0.4 = 0.52$
    \end{itemize}

    \vspace{0.3em}

    $\mathbf{e_2} \cdot P^2 = [0.18\ 0.30\ \boxed{0.52}]$ \quad (suman 1 \checkmark)

    \textbf{Respuesta a):} Hay 52\% de probabilidad de estar en C después de 2 días.
\end{frame}

\begin{frame}{Ejercicio 5: Solución Parte b) - Planteo}
    \small
    \textbf{Distribución estacionaria:} resolver $\pi P = \pi$ con $\sum \pi_i = 1$. Leyendo $\pi P = \pi$ \textbf{por columnas} de $P$:
    \[
    \begin{cases}
    \pi_A = 0.1\pi_A + 0.2\pi_B + 0.2\pi_C & \text{(col. A)} \\
    \pi_B = 0.3\pi_A + 0.2\pi_B + 0.4\pi_C & \text{(col. B)} \\
    \pi_C = 0.6\pi_A + 0.6\pi_B + 0.4\pi_C & \text{(col. C)}
    \end{cases}
    \]

    \vspace{0.3em}

    Limpiamos (pasamos términos y $\times 10$). La columna C es redundante (las tres suman $0=0$), así que la descartamos y usamos la normalización:
    \[
    \begin{cases}
...
\begin{frame}{Ejercicio 5: Solución Parte b) - Despeje}
    \small
    \textbf{Todo en función de $\pi_C$.} De $(1)$: $\pi_A = \dfrac{2\pi_B + 2\pi_C}{9}$. Lo meto en $(2)$:
    \[
    8\pi_B = \frac{6\pi_B + 6\pi_C}{9} + 4\pi_C \;\overset{\times 9}{\Longrightarrow}\; 72\pi_B = 6\pi_B + 42\pi_C \;\Rightarrow\; \pi_B = \frac{7}{11}\pi_C
    \]
    Y entonces $\pi_A = \dfrac{2\cdot\frac{7}{11}\pi_C + 2\pi_C}{9} = \dfrac{\frac{36}{11}\pi_C}{9} = \dfrac{4}{11}\pi_C$.

    \vspace{0.4em}

    \textbf{Normalización $(3)$:} $\dfrac{4}{11}\pi_C + \dfrac{7}{11}\pi_C + \pi_C = 1 \Rightarrow \dfrac{22}{11}\pi_C = 1 \Rightarrow \pi_C = \dfrac{1}{2}$.

    \vspace{0.4em}

    \[
    \pi = \left[ \frac{2}{11} \approx 0.182,\ \frac{7}{22} \approx 0.318,\ \frac{1}{2} = 0.5 \right]
    \]

    \textbf{Interpretación:} a largo plazo el ingeniero pasa $\approx 18\%$ en A, $\approx 32\%$ en B y $50\%$ en C.
\end{frame}
...
\begin{frame}{Variables de Decisión y Coste Promedio (cont.)}
    \small
...
\begin{frame}{Restricciones del Modelo LP: extraer la política}
    \small
    Una vez resuelto el LP, recuperamos la distribución estacionaria y la política a partir de las $y_{ik}$:
    \[
    ...
    $\pi_i$ es la fracción de tiempo en el estado $i$, y $D_{ik}$ es la probabilidad de tomar la decisión $k$ estando en $i$ (la política).
\end{frame}
...
\begin{frame}{Ejercicio 6: Reemplazo de Maquinaria - Decisiones}
    \small
...
\begin{frame}{Problema de Estacionamiento - Contexto (cont.)}
    \small
