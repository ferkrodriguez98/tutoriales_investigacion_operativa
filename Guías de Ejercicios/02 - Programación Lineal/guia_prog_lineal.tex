...
\toprule
\textbf{Mes} & \multicolumn{2}{c|}{\textbf{Cap. de Prod. (u/mes)}} & \multicolumn{2}{c|}{\textbf{\$ de Prod. (\$/u)}} & \textbf{\$ de Almacenaje (\$/u)} & \textbf{Demanda (u)} \\
\cmidrule(lr){2-3} \cmidrule(lr){4-5}
& H. Regulares & H. Extra & H. Regulares & H. Extra & & \\
\midrule
1 & 10 & 3 & 31 & 38 & 3 & 8 \\
2 & 8 & 2 & 32 & 38 & 3 & 10 \\
3 & 10 & 3 & 36 & 44 & 3 & 16 \\
...
\begin{tikzpicture}
\tikzset{
    vertex/.style={circle, draw, minimum size=1cm, font=\large\bfseries},
    edge/.style={->, thick}
}
...
\node[vertex] (D) at (6,1.5) {D};
\node[vertex] (E) at (6,-1.5) {E};
\node[vertex] (F) at (9,0) {F};

% Arcos con sus capacidades
\draw[edge] (A) -- node[above left] {9} (B);
\draw[edge] (A) -- node[below left] {7} (C);
\draw[edge] (B) -- node[above] {7} (D);
\draw[edge] (B) -- node[above, pos=0.3] {2} (E);
\draw[edge] (C) -- node[below, pos=0.3] {4} (D);
\draw[edge] (C) -- node[below] {6} (E);
\draw[edge] (D) -- node[right] {3} (E);
\draw[edge] (D) -- node[above] {6} (F);
\draw[edge] (E) -- node[below] {9} (F);
